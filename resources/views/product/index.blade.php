        ...
        <aside class="p-5 w-full lg:w-64 shrink-0 lg:sticky lg:top-20 lg:self-start">
            ...
            <form action="{{ route('product.index') }}" method="GET" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="departmentFilter" id="departmentFilter" class="w-full border border-gray-300 rounded-lg p-2 text-sm text-gray-600">
                ...
                <div>
                    ...
                    <select name="categoryFilter" id="categoryFilter" class="w-full border border-gray-300 rounded-lg p-2 text-sm text-gray-600">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <a href="{{ route('product.index') }}" class="w-full px-4 py-2.5 text-center border border-gray-200 hover:bg-gray-100 text-gray-600 rounded-xl text-sm font-semibold transition">Limpiar filtros</a>
                </div>
            </form>
        </aside>
                    ...
                    <article class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-col justify-between relative group hover:shadow-md transition">
                        ...
                        <a href="{{ route('product.show', $product) }}" class="h-44 rounded-xl overflow-hidden mb-4 bg-gray-50 flex items-center justify-center relative" aria-label="Ver detalles de {{ $product->title }}">
                            @if($product->images->isNotEmpty())
                                <img src="{{ asset('storage/' . $product->images->first()->rute) }}" alt="{{ $product->title }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-300">
                            @else
